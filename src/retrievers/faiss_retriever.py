...
class FAISSRetriever(BaseRetriever):
    ...
    def _setup_persistent_storage_paths(self):
        ...
        dept_faiss_dir = Path(settings.faiss_indices_dir) / self.department_name
        dept_embeddings_dir = Path(settings.embeddings_dir) / self.department_name
        ...
        print(f"Persistent storage paths configured for department: {self.department_name}")
    ...
    def _should_rebuild_index(self) -> bool:
        ...
        required_files = [self.index_file, self.embeddings_file, self.metadata_file, self.documents_file]
    ...
    def _save_index_data(self, embeddings_array):
        ...
        try:
            ...
            print(f"Index data saved to persistent storage for {self.department_name}")
    ...
    def _load_index_data(self) -> bool:
        ...
        try:
            ...
            if self.metadata_file.exists() and self.documents_file.exists():
                ...
                print(f"Metadata and documents loaded for {self.department_name}")
            else:
                return False
            ...
            print(f"✅ Successfully loaded persistent index for {self.department_name}")
            print(f"   Documents: {len(self._documents)}")
            print(f"   Index dimension: {embeddings.shape[1] if len(embeddings.shape) > 0 else 'unknown'}")
    ...
    async def _build_new_index(self, documents_path: str, start_time: float) -> None:
        ...
        print(f"Indexed {len(self._documents)} documents with dimension {dimension}")
