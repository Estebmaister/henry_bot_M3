...
from src.utils.mock_llm import MockOpenAI
...
class ResponseQualityEvaluator:
    ...
    async def evaluate_response(
        ...
        if not self._initialized:
            raise RuntimeError("Evaluator not initialized. Call initialize() first.")
        ...
        try:
            ...
            return EvaluationResult(
                overall_score=5.0,
                dimension_scores={dim: 5.0 for dim in self.evaluation_dimensions.keys()},
    ...
    def _parse_evaluation_response(self, response: str) -> EvaluationResult:
        ...
        try:
            ...
            if start_idx != -1 and end_idx != -1:
                ...
                print("Warning: Could not parse JSON from evaluation response, using defaults")
                evaluation_data = {
                    'relevance': 5.0,
                    'completeness': 5.0,
                    'accuracy': 5.0,
